// Write a program to convert an Infix expression to Prefix expression. You can use any data structure of your choice.
...
int main()
{
    ...
    reverse(infix.begin(),infix.end());  //reversing the infix expression
    for(int i=0;i<infix.length();i++)   //updating '(' to ')' and ')' to '('
    {
    	if(infix[i]=='(')
    	{
    		infix[i]=')';
    ...
    convert();  //converting the modified infix expression into postfix expression

    reverse(postfix.begin(),postfix.end());  //finally reversing the postfix expression will give us the prefix expression
    cout << "The resulting prefix expression is : " <<postfix;
    return 0;
}
